 ...
 */
class FileTransferBaseService extends \Enbit\eBaySDK\Services\BaseService
{
    /**
    ...
    public static function getConfigDefinitions()
    {
        ...
        return $definitions + [
            'apiVersion' => [
                'valid' => ['string'],
                'default' => \Enbit\eBaySDK\FileTransfer\Services\FileTransferService::API_VERSION
            ],
            'authToken' => [
                'valid' => ['string'],
                'required' => true
    ...
    protected function callOperationAsync($name, \Enbit\eBaySDK\Types\BaseType $request, $responseClass)
    {
        ...
        if ($name === 'uploadFile' && $request->hasAttachment()) {
            ...
            if (!isset($request->fileAttachment)) {
                $request->fileAttachment = new \Enbit\eBaySDK\FileTransfer\Types\FileAttachment();
            }

            if (!isset($request->fileAttachment->Data)) {
                $request->fileAttachment->Data = new \Enbit\eBaySDK\FileTransfer\Types\Data([
                    'xopInclude' => new \Enbit\eBaySDK\FileTransfer\Types\XopInclude([
