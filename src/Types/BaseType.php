...
use \Enbit\eBaySDK\Types;
use \Enbit\eBaySDK\Exceptions;
use \Enbit\eBaySDK\JmesPath\Env;
use \Enbit\eBaySDK\JmesPath\JmesPathableObjectInterface;
...
class BaseType implements JmesPathableObjectInterface
{
     ...
     *
     * @throws \Enbit\eBaySDK\Exceptions\UnknownPropertyException If the property does not exist.
     * @throws \Enbit\eBaySDK\Exceptions\InvalidPropertyTypeException If the value is the wrong type for the property.
     */
     ...
     * @param mixed $value. The value to assign to the property.
     *
     * @throws \Enbit\eBaySDK\Exceptions\InvalidPropertyTypeException If trying to assign a non array type to an repeatable property.
     */
    private function setValue($class, $name, $value)
    {
        ...
        if (!$info['repeatable']) {
            ...
            if ('array' !== $actualType) {
                throw new Exceptions\InvalidPropertyTypeException($name, 'Enbit\eBaySDK\Types\RepeatableType', $actualType);
            } else {
    ...
    private static function ensurePropertyType($class, $name, $value)
    {
        ...
        foreach ($valid as $check) {
            if ($check !== 'any' && \Enbit\eBaySDK\checkPropertyType($check)) {
    ...
    {
        if (is_subclass_of($value, '\Enbit\eBaySDK\Types\BaseType', false)) {
